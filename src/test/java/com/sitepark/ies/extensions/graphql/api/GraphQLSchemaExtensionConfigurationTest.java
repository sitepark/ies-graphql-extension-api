...
class GraphQLSchemaExtensionConfigurationTest {
	...
	void testGetSchemaBuilder() {
		assertNotNull(
				this.config.getSchemaParserBuilder(),
	...
	void testSchemaResource() {
		this.config.schemaResource(
				GraphQLSchemaExtensionConfigurationTest.class,
				"/testSchemaResource.txt");
	...
	void testDataLoader() {
		...
		assertNotNull(
				this.config.getDataLoaders().get("test"),
				"dataLoder not found");
	}
	...
	void testBatchLoader() {
		BatchLoaderWithContext<?,?> batchLoader =
				mock(BatchLoaderWithContext.class);
		this.config.batchLoader("test", batchLoader);

		assertNotNull(
				this.config.getBatchLoaders().get("test"),
				"dataLoder not found");
	}
}
